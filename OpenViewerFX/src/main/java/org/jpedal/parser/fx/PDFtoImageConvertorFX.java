...
public class PDFtoImageConvertorFX extends PDFtoImageConvertor{
    ...
    public BufferedImage pageToImage(final boolean imageIsTransparent, final PdfStreamDecoder currentImageDecoder,final float scaling, final PdfObject pdfObject) throws PdfException {
        ...
        if(Platform.isFxApplicationThread()){
            snapshot(currentImageDecoder, scaling, pdfObject, null, imageProperty);
        }else{
            Platform.runLater(new Runnable() {
                @Override public void run() {
                    snapshot(currentImageDecoder, scaling, pdfObject, latch, imageProperty);
                }
            });
        }     
    ...
    private void snapshot(final PdfStreamDecoder currentImageDecoder, final float scaling, final PdfObject pdfObject, final CountDownLatch latch, final SimpleObjectProperty<BufferedImage> imageProperty){
        try {
