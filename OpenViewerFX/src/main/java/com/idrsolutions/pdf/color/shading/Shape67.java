...
//    }
    ...
    public void generateBilinearMapping() {
        ...
        for (int i = 0; i < 4; i++) {
            float[] temp = {colorsArr[i].getRed(), colorsArr[i].getGreen(), colorsArr[i].getBlue(), colorsArr[i].getAlpha()};
            for (int j = 0; j < 4; j++) {
    ...
    public Color recurseTrapezoidal(Point2D p, Point2D[] points, Color[] colors, ArrayList<Integer> depth) {
        ...
        if (depth.size() > 2 || points[0].distance(points[1])<2) {
            return colors[0];
        }
        ...
        float[][] pointColors = new float[4][4];
        for (int i = 0; i < 4; i++) {
            float[] temp = {colors[i].getRed(), colors[i].getGreen(), colors[i].getBlue(), colors[i].getAlpha()};
            for (int j = 0; j < 4; j++) {
                pointColors[i][j] = (1f / 255f * temp[j]);
            }
        }
    ...
    private void determineSteps() {
        double crossLen = pointsArr[0].distance(pointsArr[6]);
        nSteps = crossLen < 4 ? 4 : 10;
    }
